...
class ApiClient {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    try {
      ...
      if (canRefresh && await _refreshSession()) {
        return _send(method, path, body: body, allowRefresh: false, attempt: attempt);
      }
      return _decode(response);
    } on SocketException {
      if (_retryable(method, body, attempt)) { await Future<void>.delayed(Duration(milliseconds: 400 * (attempt + 1))); return _send(method, path, body: body, allowRefresh: allowRefresh, attempt: attempt + 1); }
  ...
  bool _retryable(String method, Map<String, dynamic>? body, int attempt) =>
      attempt < 2 && (method == 'GET' || body?['idempotencyKey'] != null);
