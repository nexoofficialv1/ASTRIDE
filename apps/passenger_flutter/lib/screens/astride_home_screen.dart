...
class AstrideHomeScreen extends StatefulWidget {
  const AstrideHomeScreen({
    ...
    required this.onScanGo,
    required this.onSos,
  });

  final String Function(String) t;
  final VoidCallback onBook;
  final VoidCallback onScanGo;
...
class _AstrideHomeScreenState extends State<AstrideHomeScreen> {
  ...
  Future<void> _findMe({bool silent = false}) async {
    ...
    try {
      ...
      if (!silent && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Current location is unavailable. Check permission and try again.')),
        );
      }
    } finally {
                        ...
                        children: [
                          ...
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: widget.onScanGo,
                              icon: const Icon(Icons.qr_code_scanner_rounded),
                              label: const Text('Scan & Go'),
                            ),
                          ),
                        ],
                      ),
