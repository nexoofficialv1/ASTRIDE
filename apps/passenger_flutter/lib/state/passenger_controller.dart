...
class PassengerController extends ChangeNotifier {
  ...
  Map<String, dynamic>? activeRouteRide;
  List<Map<String, dynamic>> routeServices = [];
  bool routeAccessBusy = false;
  ...
  Future<void> bootstrap() async {
    ...
    try {
      ...
      if (session != null) {
        ...
        await refreshRouteAccess().timeout(const Duration(seconds: 25));
  ...
  String _routeIdempotencyKey(String action) {
    ...
    return 'passenger:${session?.userId ?? 'unknown'}:$action:'
        '${DateTime.now().microsecondsSinceEpoch}:$random';
  }

  Future<Map<String, dynamic>> _freshRouteLocation() async {
    if (!await Geolocator.isLocationServiceEnabled()) {
      throw ApiException('Turn on location services for Scan & Go.');
    }
    var permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied) {
      permission = await Geolocator.requestPermission();
    ...
    if (permission == LocationPermission.denied ||
        permission == LocationPermission.deniedForever) {
      throw ApiException('Location permission is required for Scan & Go.');
    }
    final position = await Geolocator.getCurrentPosition(
      locationSettings: const LocationSettings(
        accuracy: LocationAccuracy.high,
  ...
  Future<void> refreshRouteAccess() async {
    if (session == null) return;
    try {
      final output = await Future.wait([
        api.getJson('/v1/route-access/active'),
        api.getJson('/v1/route-access/routes'),
      ]);
      final active = output[0]['ride'];
      activeRouteRide = active is Map
          ? active.cast<String, dynamic>()
          : null;
      routeServices = ((output[1]['items'] ?? const []) as List)
          .whereType<Map>()
          .map((item) => item.cast<String, dynamic>())
          .toList();
      notifyListeners();
    } catch (_) {
      // Route access is optional and must not block normal booking.
    }
  }

  Future<Map<String, dynamic>> scanRouteVehicle(String vehicleQr) async {
    if (session == null) throw ApiException('Please sign in first.');
    final cleanQr = vehicleQr.trim();
    if (cleanQr.isEmpty || cleanQr.length > 2048) {
      throw ApiException('This is not a valid ASTRIDE vehicle QR.');
    }
    if (routeAccessBusy) throw ApiException('A Scan & Go request is already processing.');
    routeAccessBusy = true;
    notifyListeners();
    try {
      final location = await _freshRouteLocation();
      final response = await api.postJson('/v1/route-access/scan', {
        'vehicleQr': cleanQr,
        'location': location,
        'idempotencyKey': _routeIdempotencyKey('scan'),
      });
      final ride = response['ride'];
      activeRouteRide = response['action'] == 'ENDED'
          ? null
          : (ride is Map ? ride.cast<String, dynamic>() : activeRouteRide);
      await restoreActiveBooking();
      await refreshRouteAccess();
      return response;
    } finally {
      routeAccessBusy = false;
      notifyListeners();
    }
  }

  Future<Map<String, dynamic>> endRouteRide() async {
    if (activeRouteRide == null) {
      throw ApiException('No active Scan & Go ride was found.');
    }
    if (routeAccessBusy) throw ApiException('A Scan & Go request is already processing.');
    routeAccessBusy = true;
    notifyListeners();
    try {
      final location = await _freshRouteLocation();
      final response = await api.postJson('/v1/route-access/end', {
        'location': location,
        'idempotencyKey': _routeIdempotencyKey('end'),
      });
  ...
  Future<Map<String, dynamic>> rechargeRideCredit(
    PaymentGateway gateway,
    int amountPaise,
  ) async {
    if (amountPaise < 1000 || amountPaise > 5000000) {
      throw ApiException('Recharge must be between ₹10 and ₹50,000.');
    }
    final created = await api.postJson('/v1/passenger/wallet/add-money', {
      'amountPaise': amountPaise,
    });
    final order = ((created['order'] ?? const {}) as Map)
        .cast<String, dynamic>();
    final providerOrderId =
        '${order['providerOrderId'] ?? created['providerOrderId'] ?? ''}';
    if (providerOrderId.isEmpty) {
      throw ApiException('Verified recharge order was not returned.');
    }
    String keyId = AppConfig.razorpayKeyId;
    try {
      final publicConfig = await api.getJson(
      ...
      final providers = publicConfig['clientProviders'];
      final payments = providers is Map ? providers['payments'] : null;
      if (payments is Map) keyId = '${payments['razorpayKeyId'] ?? keyId}';
    } catch (_) {}
    final result = await gateway.open(
      keyId: keyId,
      orderId: providerOrderId,
      amountPaise: amountPaise,
      passengerName: '${profile['fullName'] ?? profileName}',
      passengerMobile: session?.mobile ?? '',
      description: 'ASTRIDE Ride Credit recharge',
    );
    return api.postJson('/v1/passenger/wallet/payment/verify', {
      'topupReference': created['topupReference'],
      'providerOrderId': result.orderId,
      'providerPaymentId': result.paymentId,
      'signature': result.signature,
      'amountPaise': amountPaise,
  ...
  Future<Map<String, dynamic>> holdRideCreditForBooking(String bookingId) =>
      api.postJson('/v1/passenger/wallet/pay-ride', {'bookingId': bookingId});
